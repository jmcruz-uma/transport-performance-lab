 ...
 *
 * TCP file server with Asio over TLS 1.3, length-prefixed framing
 * (scenario "tls_framed").
 *
 * Same shape as ../tcpserver_tls/server.cpp (asio::ssl::stream, server
 * handshake, mmap'd payload, thread pool) with framing added: instead of one
 * write loop over the whole payload, the server sends N discrete messages whose
 * sizes come from the shared manifest (tls/manifest.txt, path in TLS_MANIFEST).
 * Each message goes out as a single gather write of {4-byte big-endian length,
 * body}; the body bytes are slices of the mmap'd payload, walked in order and
 * wrapped around, so the wire stays deterministic. This is byte-for-byte the
 * frame format taps_cpp's LengthPrefixedFramer produces, so the TAPS arm and
 * this one exchange identical wire data. After the last message the stream is
 * shut down, which the client sees as end-of-stream.
 */
...
static asio::awaitable<void> send_messages(tls_stream& stream, std::span<const char> payload,
    ...
    for (std::size_t size : sizes) {
        ...
        co_await asio::async_write(stream, frame, asio::use_awaitable);
        offset += size;
    }
    co_return;
}
...
static asio::awaitable<void> serve_client(tls_stream stream, std::span<const char> payload,
    ...
    try {
        ...
        co_await send_messages(stream, payload, sizes);
        asio::error_code ec;
    ...
    while (true) {
        try {
            ...
            asio::co_spawn(executor,
                           serve_client(tls_stream(std::move(socket), ssl_ctx), payload, sizes),
                           asio::detached);
        } catch (...) {
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        asio::co_spawn(io_context, accept_loop(acceptor, ssl_ctx, payload, sizes), asio::detached);
