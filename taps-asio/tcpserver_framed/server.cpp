 ...
 *
 * TCP file server with TAPS, length-prefixed framing, NO security layer
 ...
 * -- PassthroughFramer's parse() can only ever emit after the connection's
 * half-close, so it was structurally incapable of the incremental delivery
 * "blocks" wanted to measure. LengthPrefixedFramer already does real
 * incremental delivery -- its parse() ignores at_eof and emits the instant a
 * full record has arrived -- and "tls_framed" already proves it end to end.
 * This scenario is that same design with TLS removed).
 *
 * Same shape as ../tcpserver/server.cpp (one connection.send() per message)
 * with framing added: the connection carries a LengthPrefixedFramer and the
 * server sends N discrete Messages whose sizes come from the shared manifest
 ...
 * is TLS-specific). The message bodies are slices of the mmap'd payload,
 * walked in order and wrapped around, so the wire bytes stay deterministic.
 * After the last message the connection is closed, which the client sees as
...
static asio::awaitable<void> listen_loop(
    ...
    std::cout << "TAPS framed server listening on port " << port << "\n";
